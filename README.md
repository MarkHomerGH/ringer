...
| `check_timeout_s` | Per-attempt budget for the check command (default 60 s), independent of `timeout_s` |
...
**check script:** Lint refuses a manifest when a check runs a script file that does not exist. The script is the first word after an interpreter (`sh`, `bash`, `zsh`, `node`, `python*`, `source`, or `.`), or a bare absolute first word; `env` and `exec` are wrappers that lint looks through (`env` skips `-u`/`-C`/`--unset`/`--chdir` operands), but other wrappers such as `sudo`, `timeout`, `nice`, or `xargs` are not looked through, so their script operands are unfenced. Redirect targets, `cd`/`cp` operands, `--flag` values, `/dev/null`, interpreter binaries, data files, and `{placeholders}` are deliberately not fenced; a path the shell still has to build (`$VAR`, glob, `$(...)`, backticks) is reported as unresolved and not fenced, and text after a heredoc operator is not analysed. A relative script path gets an advisory because it resolves inside the worker's own writable folder unless the check first does `cd /abs`.

`check_samples` let a task test its own check before workers spend tokens: each sample copies one or more files into a throwaway folder, expects the check to `pass` or `fail`, can carry an optional `note`, can supply `args` as shell text appended verbatim to the end of the check for that sample only (refused when the check is chained), and can require `fail_contains` for expected failures; sample paths should be absolute or `~/...` because a relative path resolves against the directory lint is run from. Standalone `lint` and the implicit lint at `run` and `run --dry-run` execute samples under the task's `check_timeout_s` and refuse the run on a mismatch before any worker starts, printing `lint: sample <task>/<label>: <command>` first; samples are skipped while any `{{` placeholder remains. Sample-test checks whose truth comes from files in the working directory, not checks that read git state, `$PWD` outside the folder, or a patch they export.

At run start, Ringer fingerprints each check script it fenced at lint and re-checks that check script before every check script execution; a changed or missing check script fails that task without running the check script and without retry, and the row is not counted against the model, with the operator told to restart the run. This covers ONLY the single check script file the check script command directly executes: not inline checks, not files the check script imports or sources, not relative scripts, not input files passed as flag values, and not an edit that lands while the check script is already executing.
...
The scoreboard reports, per model and task_type: tasks, attempts, `pass_rate`, `first_try_pass_rate`, median duration and token count, and `last_seen`. The signal for routing is `first_try_pass_rate` — the share of tasks that passed on attempt 1 without a retry; `pass_rate` is the rescued rate after Ringer's single retry, so the gap between the two is the cost of the retry lane. `Non-model` counts tasks with at least one attempt whose verdict came from something other than the worker's output (today: the boss check timing out — recorded as `cause` on every model-log row, values `worker-output` / `check-timeout`). Those attempts are ignored when choosing a task's first and final verdict, and a task with only such attempts is left out of the rates entirely. Slice the log with `--log` (a different JSONL), `--task-type`, `--model`, `--engine`, `--since`, or `--json` for piping elsewhere.
...
- [@MarkHomerGH](https://github.com/MarkHomerGH) (Mark Homer) — maintains this fork: Ringside (the `hud` scoreboard), `models` evidence and the per-model notes in `docs/MODEL-NOTES.md`
